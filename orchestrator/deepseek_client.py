...
class DeepSeekClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.deepseek.com",
        ...
        self.client = OpenAI(
            ...
            base_url=base_url,
            timeout=timeout,
            max_retries=0,  # 禁用SDK内置重试，使用手动重试
        )
    ...
    def _retry_call(self, fn, description: str = "API"):
        ...
        通用重试包装器
        Args:
            fn: 无参可调用对象, 执行实际的API请求
            description: 用于日志输出的描述
        """
        retry_delay = 2
        last_exception = None
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if is_retryable and attempt < self.max_retries - 1:
                    ...
                    print(f"   [!] {description} 连接错误, {wait}s 后重试 "
    ...
    def chat(self, messages: list[dict], temperature: float = 0.3,
        ...
        原理: 服务器一边生成一边发送小块数据, 不会因为
        "等整个响应生成完再一次性发送" 而被中间网络设备断连。
        """
        def _do_stream():
    ...
    def chat_with_tools(self, messages: list[dict], tools: list[dict],
        ...
        tool_calls 响应通常较短, 断连概率低; 若仍断连则重试。
        """
        def _do_call():
